...
class HTTP 
{
    ...
    /**
     * Avoids reusing any currently-open connections to hosts on future requests. 
     * Returns the hostnames for which we would have attempted to reuse connections.
     *
     * @return string[]
     */
    ...
    /**
     * CURL 핸들 닫기.
     * register_shutdown_function() 으로 자동 등록된다.
     * Closes our current curl handle. 
     * Happens automatically on shutdown, normally no need to call this.
     */
    ...
    public static function get( string $url, $fileHandle = null, ?string $range = null ) 
    {
        ...
        else 
        {
            ...
            } while ( $data && preg_match( '/^HTTP\/\d+\.\d+ (\d+)/', $headerLines[0], $res ) && ( $res[1] != $responseCode ) ); // mostly to handle 100 Continue, maybe 30x redirects too
        }
    ...
    public static function head( string $url ) : array 
    {
        ...
        $data   = curl_exec( $ch );     // 웹요청
        $errMsg = curl_error( $ch );    // 웹에러
    ...
    private static function onCurlProgress( int $totalDown, int $transmittedDown ) : void 
    {
        ...
        if ( $totalDown < static::PROGRESS_MINIMUM_FILE_SIZE ||
